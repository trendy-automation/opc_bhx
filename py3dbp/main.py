...
class Packer:
    ...
    def pack2Bin(self, bin, item,fix_point):
        ''' pack item to bin '''
        #print('pack2Bin bin.items',bin.items,'bin.corner',bin.corner,'item',item.partno,str(item.position))
        fitted = False
        bin.fix_point = fix_point

        # first put item on (0,0,0) , if corner exist ,first add corner in box. 
        if bin.corner != 0 and not bin.items:
            #print('bin.corner != 0 and not bin.items = ',bin.corner != 0 and not bin.items)
        ...
        for axis in range(0, 3):
            ...
            for ib in items_in_bin:
                ...
                #print('item', item.partno, str(item.position),'pivot', pivot, 'axis', axis)
            ...
            if fitted:
                break
        if not fitted:
            #print('bin.unfitted_items.append(item)')
            bin.unfitted_items.append(item)
    ...
    def pack(self, bigger_first=False,distribute_items=False,fix_point=True,binding=[],number_of_decimals=DEFAULT_NUMBER_OF_DECIMALS):
        try:
            ...
            return dict(error=f"Ошибка упаковки слоя {str(error)} {traceback.format_exc()}")
...
class Painter:
    ...
    def _plotCube(self, ax, x, y, z, dx, dy, dz, color='red',mode=2):
        ...
        if mode == 1 :
            ...
            p = Rectangle((x,y),dx,dy,fc=color,ec='black', alpha=0.5)
            p2 = Rectangle((x,y),dx,dy,fc=color,ec='black', alpha=0.5)
            p3 = Rectangle((y,z),dy,dz,fc=color,ec='black', alpha=0.5)
            p4 = Rectangle((y,z),dy,dz,fc=color,ec='black', alpha=0.5)
            p5 = Rectangle((x,z),dx,dz,fc=color,ec='black', alpha=0.5)
            p6 = Rectangle((x,z),dx,dz,fc=color,ec='black', alpha=0.5)
            ax.add_patch(p)
            ax.add_patch(p2)
            ax.add_patch(p3)
            ax.add_patch(p4)
    ...
    def save_plot(self, name="image.png") -> str:
        """ side effective. Plot the Bin and the items it contains. """
        fig = plt.figure()
        axGlob = plt.axes(projection='3d')
    ...
    def get_plot(self):
        ...
        self.setAxesEqual(axGlob)
        io_buf = io.BytesIO()
        FigureCanvasAgg(fig).print_png(io_buf)
        plt.close()
